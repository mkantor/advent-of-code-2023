...
def maxCapturedValue(pattern: Regex, source: CharSequence) =
  pattern
...
def minCounts(revelations: String) =
  CubeCounts(
    maxCapturedValue(redPattern, revelations),
    maxCapturedValue(greenPattern, revelations),
    maxCapturedValue(bluePattern, revelations)
  )

def part1(input: String) = {
  val target = CubeCounts(12, 13, 14)
  input
    .split('\n')
    .collect { case gamePattern(gameNumber, revelations) => (gameNumber, revelations) }
    .filter { (_, revelations) =>
      {
        val min = minCounts(revelations)
        target.red >= min.red && target.green >= min.green && target.blue >= min.blue
      }
    }
    .map(_._1.toInt)
    .sum
...
val input = """Game 1: 7 blue, 6 green, 3 red; 3 red, 5 green, 1 blue; 1 red, 5 green, 8 blue; 3 red, 1 green, 5 blue
